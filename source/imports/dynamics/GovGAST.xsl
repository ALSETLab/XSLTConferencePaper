...
<xsl:stylesheet version="2.0" 
	...
	<xsl:template match="cim:GovGAST/cim:TurbineGovernorDynamics.SynchronousMachineDynamics">
		...
		<xsl:value-of select="format-number(../cim:GovGAST.r,'0.000000000#')"/>
		<xsl:text>,T_1 = </xsl:text>
		<xsl:value-of select="format-number(../cim:GovGAST.t1,'0.000000000#')"/>
		<xsl:text>,T_2 = </xsl:text>
		<xsl:value-of select="format-number(../cim:GovGAST.t2,'0.000000000#')"/>
		<xsl:text>,T_3 = </xsl:text>
		<xsl:value-of select="format-number(../cim:GovGAST.t3,'0.000000000#')"/>
		<xsl:text>,AT = </xsl:text>
		<xsl:value-of select="format-number(../cim:GovGAST.at,'0.000000000#')"/>
		<xsl:text>,K_T = </xsl:text>
		<xsl:value-of select="format-number(../cim:GovGAST.kt,'0.000000000#')"/>
		<xsl:text>,V_MAX = </xsl:text>
		<xsl:value-of select="format-number(../cim:GovGAST.vmax,'0.000000000#')"/>
		<xsl:text>,V_MIN = </xsl:text>
		<xsl:value-of select="format-number(../cim:GovGAST.vmin,'0.000000000#')"/>
		<xsl:text>,D_turb = </xsl:text>
		<xsl:value-of select="format-number(../cim:GovGAST.dturb,'0.000000000#')"/>
