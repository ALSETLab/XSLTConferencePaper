...
<xsl:stylesheet version="2.0" 
	...
	<xsl:template match="cim:LinearShuntCompensator">
		...
		<xsl:text>OpenIPSL.Electrical.Banks.PSSE.Shunt </xsl:text><xsl:value-of select="gkh:equipmentLoadName(@rdf:ID)"/>
		<xsl:text>(G = </xsl:text><xsl:value-of select="format-number(cim:LinearShuntCompensator.gPerSection * $baseImpedance,'0.0000000000#')"/>
		<xsl:text>,B = </xsl:text><xsl:value-of select="format-number(cim:LinearShuntCompensator.bPerSection * $baseImpedance,'0.0000000000#')"/>
		<xsl:text>);
		</xsl:text>
	</xsl:template>
