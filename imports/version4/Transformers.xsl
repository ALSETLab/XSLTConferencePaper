...
<xsl:text>OpenIPSL.Electrical.Branches.PSSE.TwoWindingTransformer </xsl:text><xsl:value-of select="gkh:compliantName(concat(gkh:transformerName($transformerCode,''),cim:PowerTransformerEnd.PowerTransformer/substring(@rdf:resource,7,4)))"/>
<xsl:text>(R = </xsl:text><xsl:value-of select="format-number(cim:PowerTransformerEnd.r div $baseImpedance,'0.000000000#')"/>
<xsl:text>, X = </xsl:text><xsl:value-of select="format-number(cim:PowerTransformerEnd.x div $baseImpedance,'0.000000000#')"/>
<xsl:text>, G = </xsl:text><xsl:value-of select="format-number(cim:PowerTransformerEnd.g * $baseImpedance div 2,'0.000000000#')"/>
<xsl:text>, B = </xsl:text><xsl:value-of select="format-number(cim:PowerTransformerEnd.b * $baseImpedance div 2,'0.000000000#')"/>
