...
<xsl:include href="dynamics/SynchronousMachineTimeConstantReactance.xsl"/>
<xsl:include href="dynamics/HydroGen.xsl"/>
...
<xsl:text>
model </xsl:text><xsl:copy-of select="$GName"/>
<xsl:text>
...
// Writing machine;
		...
		<xsl:call-template name="MakeMachineForCode">
			...
			<xsl:with-param name="hydroCode" select="cim:RotatingMachine.GeneratingUnit/substring(@rdf:resource,2)"/>
			<xsl:with-param name="GenName" select="$GName"/>
		</xsl:call-template>
	</xsl:template>
	<xsl:template name="MakeMachineForCode">
		<xsl:param as="xs:string" name="code"/>
		<xsl:param as="xs:string" name="hydroCode"/>
		...
		<xsl:choose>
			...
			<xsl:when test="$rdf/cim:HydroGeneratingUnit[@rdf:ID=$hydroCode]">
				<xsl:apply-templates select="$rdf/cim:HydroGeneratingUnit[@rdf:ID=$hydroCode]">
					<xsl:with-param name="MainName" select="$GenName"/>
				</xsl:apply-templates>
			</xsl:when>
			<xsl:otherwise>
				<xsl:text>GENERATING UNITS THAT IS NOT IN SYSTEM!</xsl:text>
			</xsl:otherwise>
		</xsl:choose>
<xsl:text>
end </xsl:text><xsl:copy-of select="$GenName"/><xsl:text>;</xsl:text>
	</xsl:template>
